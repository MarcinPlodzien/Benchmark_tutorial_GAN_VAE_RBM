...
# ------------------------------------------------------------------------------
# ==============================================================================
# TARGET DISTRIBUTION ZOOLOGY
# ==============================================================================
# Defines the ground truth probability distribution P(x) we want to learn.
# TARGET_TYPE = "1D_TRIMODAL" 

TARGET_TYPE = "2D_MIX_GAUSS" 

if TARGET_TYPE == "2D_MIX_GAUSS":
...
# If NUM_BINS > 0, we discretize the 1D continuous signal into a One-Hot Vector.
# The 'Visible Layer' then becomes size NUM_BINS.
...
def sample_target(key, batch_size):
    ...
    """
    # 1. Generate Continuous 1D Raw Sample
    raw = jnp.zeros((batch_size, 1))
    
    if TARGET_TYPE == "2D_MIX_GAUSS":
        # 2D Special Case - NO Discretization support in this version for simplicity
        k1, k2 = jax.random.split(key)
        mode_indices = jax.random.randint(k1, shape=(batch_size,), minval=0, maxval=4)
        centers = MEANS[mode_indices]
        noise = jax.random.normal(k2, shape=(batch_size, 2)) * SIGMA
        return centers + noise
        
    elif TARGET_TYPE in ["1D_BIMODAL", "1D_TRIMODAL", "1D_QUADMODAL"]:
        # Generalized N-Mode 1D Mixture
        n_modes = MEANS.shape[0]
        k1, k2 = jax.random.split(key)
        mode_indices = jax.random.randint(k1, shape=(batch_size,), minval=0, maxval=n_modes)
        centers = MEANS[mode_indices] 
        noise = jax.random.normal(k2, shape=(batch_size, 1)) * SIGMA
        raw = centers + noise
        
    elif TARGET_TYPE == "1D_BETA":
        raw = (jax.random.beta(key, BETA_A, BETA_B, shape=(batch_size, 1)) - 0.5) * 5.0 
        
    elif TARGET_TYPE == "1D_POISSON":
        raw = jax.random.poisson(key, POISSON_LAMBDA, shape=(batch_size, 1)).astype(jnp.float32) - POISSON_LAMBDA

    # 2. Handles Discretization (Binning)
    if NUM_BINS > 0:
        # Define Bin Edges covering the active range [-3.5, 3.5]
        # Bins capture values outside range in first/last bin
        bins = jnp.linspace(-3.5, 3.5, NUM_BINS + 1)
        
        # Digitize: Returns indices 0..NUM_BINS (where 0 is < lowest edge, NUM_BINS is > highest)
        # We clamp to 0..NUM_BINS-1 to ensure valid one-hot indices
        indices = jnp.digitize(raw, bins) - 1
